...
function LilyPadTrail({ total, current, reachedShore, feedback }) {
  return (
      ...
      <div className="flex items-center gap-2 overflow-x-auto">
        {Array.from({ length: total }).map((_, i) => {
          ...
          return (
            <div
              key={i}
              className={`flex items-center justify-center w-10 h-10 rounded-full shrink-0 text-lg transition-colors ${
                isDone ? "bg-emerald-600" : isCurrent ? "bg-emerald-400" : "bg-slate-700"
              }`}
            >
              {isCurrent ? (
                <img 
                  src={feedback === "correct" ? "/studymons/jogoSapo/sapo2.png" : "/studymons/jogoSapo/sapo.png"} 
                  alt="Sapo" 
                  style={{ 
                    width: '32px', 
                    height: '32px', 
        ...
        >
          {reachedShore && (
            <img 
              src="/studymons/jogoSapo/sapo.png" 
              alt="Sapo Vencedor" 
...
export default function Ciensapo({ onBack }) {
  ...
  return (
    <div 
      ...
      style={{
        backgroundImage: 'url("/studymons/jogoSapo/fundoAGUA.gif")',
    ...
    >
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <button
            onClick={onBack}
            className="text-slate-200 hover:text-white text-sm underline transition bg-black/30 px-3 py-1 rounded-full"
          >
            Voltar ao Hub
          </button>
          <h1 className="text-3xl md:text-4xl font-black tracking-widest text-teal-300 drop-shadow-[0_0_10px_rgba(0,0,0,0.8)]">
            CIENSAPO
          </h1>
          <div className="flex gap-1 text-xl bg-black/30 px-3 py-1 rounded-full">
        ...
        {/* Regras */}
        <div className="bg-slate-900/95 border border-slate-700 rounded-xl p-4 mb-6 text-xs sm:text-sm text-slate-300 shadow-lg">
          <p className="font-bold text-white mb-1">Como jogar:</p>
          <p>
        ...
        {/* Trilha de progresso passando o feedback */}
        <LilyPadTrail
        ...
        {/* Fim de jogo */}
        {gameStatus === "won" && (
          <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl p-6 text-center mb-6 shadow-2xl border-4 border-emerald-300">
            <h2 className="text-2xl font-black mb-1 text-white">CHEGOU À MARGEM!</h2>
            <p className="mb-4 font-medium text-emerald-50">O sapo atravessou o rio respondendo tudo certinho!</p>
            <button
        ...
        {gameStatus === "lost" && (
          <div className="bg-gradient-to-br from-red-600 to-rose-700 rounded-xl p-6 text-center mb-6 shadow-2xl border-4 border-red-300">
            <h2 className="text-2xl font-black mb-1 text-white">O SAPO CAIU NO RIO!</h2>
            <p className="mb-4 font-medium text-red-50">Suas vidas acabaram antes de chegar à margem.</p>
            <button
              onClick={startNewGame}
        ...
        {/* Pergunta ativa */}
        {gameStatus === "playing" && (
          <div className="bg-slate-900/95 rounded-2xl shadow-xl p-6 border border-slate-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {currentQuestion.options.map((option, i) => {
                ...
                let styles = "bg-slate-800 hover:bg-teal-900 text-slate-100 border-slate-600";
                ...
                return (
                  <button
                    ...
                    disabled={!!feedback}
                    className={`${styles} font-semibold py-6 px-4 rounded-xl transition-all duration-200 disabled:cursor-not-allowed border-2`}
                  >
                    {option}
                  </button>
                );
            ...
            {feedback && (
              <p
                className={`text-center text-lg font-black mt-6 animate-bounce ${
